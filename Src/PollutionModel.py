...
from Data.Generators.AdDifGenerator import generateEqual, trueSolution
...
def showErrorMap(t, outputNumber=0, n=10):
    with torch.no_grad():
        ...
        for i in range(n):
            ...
            for j in range(n):
                tNet[j] = t
                rTrue.append(trueSolution(gridX[i, j], gridY[i, j], t))
            tNet = tNet.reshape(n, 1)
            tNet = torch.from_numpy(tNet).float()
    ...
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title(["contamination","k-coefficient","sourceterm","diffusion coefficient 1","diffusion coefficient 2"][outputNumber] + " - t = " + str(t))
...
def f(x, y, t):
    ...
    d1_x = nth_derivative(flat(netOutput[:, 3]), wrt=x, n=1)
    d2_y = nth_derivative(flat(netOutput[:, 4]), wrt=y, n=1)
    ...
    f = u_t + vx(x, y, t, T0) * u_x + vx_x(x, y, t, T0) * netOutput[:,
        ...
        x, y, t, T0) * netOutput[:, 0].reshape(1,
                                               -1).t() - d1_x * u_x - netOutput[:,3].reshape(1, -1).t() * u_xx - d2_y * u_y - netOutput[:, 4].reshape(1,
                                                                 -1).t() * u_yy - netOutput[
                                                                                  :,
                                                                                  2].reshape(
        1, -1).t() + \
        netOutput[:, 1].reshape(1, -1).t() * netOutput[:, 0].reshape(1, -1).t()
