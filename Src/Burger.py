...
ix1 = torch.zeros(vxn, dtype=torch.float, requires_grad=True).reshape(vxn,
                                                                      1) + 1.0
ixm1 = torch.zeros(vxn, dtype=torch.float, requires_grad=True).reshape(vxn,
                                                                       1) - 1.0
...
ixm1t = torch.from_numpy(u_true[0, :]).float().reshape(4, 10,
                                                       1)  # boundary u values for x = -1
ix1t = torch.from_numpy(u_true[vxn - 1, :]).float().reshape(4, 10,
                                                            1)  # uvals for x = 1
...
zeros = torch.zeros(vtn, dtype=torch.float, requires_grad=True).reshape(4, 10,
                                                                        1)

m1 = list(zip(mx, mt, mu))
m2 = list(zip(mx2[1:4], mt2[1:4],
              ixm1t[1:4]))  # the x=-1 boundary tripple (-1,t, u(-1,t))
m3 = list(zip(mx3[1:4], mt2[1:4],
              ix1t[1:4]))  # the x=1 boundary tripple (1,t, u(1,t))
...
for epoc in range(1, epocs + 1):
    ...
    if epoc % 500 == 0:
        ...
        epoc, float(loss), float(loss2tot)), file=open('./burger_out.txt', 'a'))
    if epoc % 5000 == 0:
